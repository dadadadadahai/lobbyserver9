            ...
            if gamePool.poolType == 5 then
                for gameType, gameTypeInfo in pairs(gamePool.gameTypeInfos) do
                    ...
                    if bBomb then
                        ...
                        if (gameTypeInfo.realPoolChips < math.floor(gameTypeInfo.fakePoolChips * (scaleConfigs[#scaleConfigs].fakePoolPer / 100)) + maxChips) or (not realPoolFlag) then
...
function GetGamePoolChips3(gameId, gameType, betIndex)
    ...
    if gameTypeInfo == nil then
        ...
        return false, jackpotNum, jackpotChips,jackpotMul
    end
    ...
    -- 爆池倍数
    jackpotMul = realScaleConfig.mul
    --可以爆池了
    jackpotNum = gameTypeInfo.jackpotNum
    jackpotChips = gameTypeInfo.realChips

    -- print("jackpotChips = "..jackpotChips)
    -- print("jackpotMul = "..jackpotMul)
...
function ReducePoolChips(gameId,gameType,jackpotChips)
    ...
    -- 真奖池爆池减少假奖池金额
    gameTypeInfo.fakePoolChips = gameTypeInfo.fakePoolChips - math.floor(jackpotChips)
    -- 真奖池爆池减少真奖池金额
    gameTypeInfo.realPoolChips = gameTypeInfo.realPoolChips - math.floor(jackpotChips)
end
...
function GamePoolsTenTick3()
    ...
    for gameId, gamePool in pairs(GAME_POOL_CONFIG3) do
        for gameType, gameTypeInfo in pairs(gamePool.gameTypeInfos) do
            ...
            --真奖池如果没人中奖， 扣除假奖池数量
            if gameTypeInfo.realChips > 0 and curTime >= gameTypeInfo.realEndTime then
                local params = {
