...
unilight.islobbyserver = function()
    return go.config().GetConfigBool("lobby_server")
end
...
--自定义路径遍历
unilight.customfiles = function(path)
	return luar.slice2table(go.getLuaFiles(path))
end
...
unilight.error = function(...)
	...
	go.logging.Error(...)
	--go.logging.Error(debug.traceback())
end
...
unilight.success = function(w, req)
	...
	if req.data ~= nil and req.data.errno == nil then
...
--发送消息给指定玩家
unilight.sendcmd = function(uid, req)
    local laccount = go.roomusermgr.GetRoomUserById(uid)
    if laccount ~= nil then
        unilight.success(laccount, req)
    else
        unilight.error(string.format("发送消息客端不在线:%d,msg=%s",uid, table2json(req)))
    end
end
...
unilight.response = function(w, req)
    ...
    if nu ~= nil then 
	    ...
	    req.st = os.msectime() + unilight.tzoffset()
	    w.SendString(s)
...
unilight.redisdbready = function(handler)
	...
	-- 这里不需要去创建表
	-- print('go.config().GetConfigStr("zone_key")',go.config().GetConfigStr("zone_key"))
	--all
	unilight.REDISDB.SubChannel(go.config().GetConfigStr("zone_key"))
end
--收到redis订阅消息
unilight.recvRedisChannel=function (channel,data)
	print('recv redis pub',data)
	if data=='1' or data=='2' then
		return 
	end
	local dobj = json.decode(data)
	if dobj.c=='chargeBack' then
		--收到充值成功通知
		local orderId = dobj.orderId
		local uid = dobj.uid
		chessrechargemgr.CheckOrderDeliveryNew(uid,orderId)
	elseif dobj.c=='drawCashBack' then
		--收到提现成功通知
		local orderId = dobj.orderId
		local uid = dobj.uid
	end
	...
	__newindex = function(t, k, handle)
		if handle == nil then
			...
			Lobby[k] = function(reqdata, w, msgname)
				...
				--unilight.info("lobby msg:"..table2json(req))
				local r0, r1 = handle(req, w)
	...
	__newindex = function(t, k, handle)
		if handle == nil then
			...
			req.msgtype = "lobbymessage"
				...
				--unilight.info("zone msg:"..table2json(req))
				local r0, r1 = handle(req, w)
				--req.msgtype = "lobbymessage"
				if w == nil then
					return r0							-- return r0 for server test
