...
unilight.initmongodb = function()
	...
	unilight.getdataNoCatch = function (name,key)
		if key == nil then
			return nil
		end
	...
	unilight.getdata = function(name, key, force)
		...
		--优先从本地缓存中获取
		if mongo_data_cache[name]~=nil and mongo_data_cache[name][key] ~= nil then
			--从本地缓存获取信息
			return mongo_data_cache[name][key].data
		elseif Const.REDIS_CACHE_NAME[name] ~= nil and unilight.REDISDB ~= nil then
			--从redis获得
			-- local resData = unilight.redis_gethashdata_Str(name, tostring(key))
			local resData = unilight.redis_getdata(name .. '_' .. tostring(key))
			-- unilight.info('redis resdata:'..key)
			if resData ~= "" then
				-- unilight.info('get redis data:'..key)
				-- unilight.info("redis 获取玩家信息:"..key)
				local data = json2table(resData)
				local laccount = go.roomusermgr.GetRoomUserById(key)
				if laccount~=nil then
					mongo_data_cache[name] = mongo_data_cache[name] or {}
					mongo_data_cache[name][key]={data=data,lastsavetime=0}
				end
				return data
			end
		end
		...
		if name ~= "data" or cache == nil  then
            ...
            if Const.REDIS_CACHE_NAME[name] ~= nil and unilight.REDISDB ~= nil and force == nil  then
                -- unilight.redis_sethashdata(name, tostring(key), table2json(resData))
				-- unilight.redis_setdata(name..'_'..tostring(key),table2json(resData))
				-- unilight.redis_setexpire(name..'_'..tostring(key),1800)
				unilight.REDISDB.ExecuteCommand('SETEX',name..'_'..tostring(key),1800,table2json(resData))
				if mongo_data_cache[name]==nil then
					mongo_data_cache[name]={}
				end
				local laccount = go.roomusermgr.GetRoomUserById(key)
				if laccount~=nil then
					mongo_data_cache[name][key]={
						lastsavetime = os.time(),
						data = resData,
					}
				end
                -- unilight.info("redis 保存玩家信息:"..key)
            end
            return resData
		end
		return cache.data
	...
	unilight.savedata = function(name, data, force, nocache)
		data.autoInsertTime = os.time()
		-- print('savedata test')
		if data._id~=nil and nocache == nil then
			if storecatch.Save(name,data,force) then
				-- print('storecatch return')
				return
			end
		end
        if Const.REDIS_CACHE_NAME[name] ~= nil and unilight.REDISDB ~= nil and data._id~=nil then
            if true then
                --优先保存redis
                -- unilight.redis_sethashdata(name, tostring(data._id), table2json(data))
				-- unilight.redis_setdata(name..'_'..tostring(data._id),table2json(data))
                -- print(table2json(data))
				-- print('ExecuteCommand setex')
				-- local _,e = unilight.REDISDB.ExecuteCommand('SETEX',name..'_'..tostring(data._id),1800,table2json(data))
				-- if e~=nil then
				-- 	unilight.error("set redis error")
				-- end
				local laccount = go.roomusermgr.GetRoomUserById(data._id)
				if laccount~=nil then
					local cur_time = os.time()
                	if mongo_data_cache[name] == nil then mongo_data_cache[name] = {} end
                	if mongo_data_cache[name][data._id] == nil then 
						mongo_data_cache[name][data._id] = {lastsavetime = cur_time}
					end
					...
					--本地缓存更新
					mongo_data_cache[name][data._id].data = data
					if force==nil then
						return
					end
				end
		...
		if r.E ~= nil then
			unilight.error("unilight.savedata:"..name..", data="..table2json(data))
			unilight.error(r.E)
	...
	unilight.savedatasyn = function(name, data,force)
        -- unilight.get_mongodb().SaveCollectionSynById(name, encode_repair(data))
		unilight.get_mongodb().SaveCollectionById(name, encode_repair(data))
	end
	unilight.updateNoEncode=function (name,id,args)
		local r = unilight.get_mongodb().SaveCollectionFieldById(name, id,"", args)
		if r ~= nil and id ~= nil then
			local errorInfo = "unilight.updateNoEncode :" .. tostring(r)..'name:'..name
			if id ~= nil then
	...
	unilight.updateNoCatch = function(name, id, args)
		if id == nil or type(id) == "userdata" or args == nil or type(args) == "userdata" then
			unilight.error(" updateNoCatch id or data is null or type() is userdata")
		...
		if r ~= nil and id ~= nil then
			local errorInfo = "unilight.update :" .. tostring(r)..'name:'..name
			if id ~= nil then
				errorInfo = errorInfo .. " _id:" .. id .. "   type is" .. type(id)
			end
			unilight.error(errorInfo)
	...
	unilight.update = function(name, id, args)
		args.autoInsertTime = os.time()
		if storecatch.Update(name,id,args) then
			return
		end
		if id == nil or type(id) == "userdata" or args == nil or type(args) == "userdata" then
			unilight.error(" update id or data is null or type() is userdata")
			return "data type error "  
		end
		
		--更新redis缓存和本地缓存
		local laccount = go.roomusermgr.GetRoomUserById(id)
		if Const.REDIS_CACHE_NAME[name] ~= nil and laccount~=nil then
			mongo_data_cache[name] = mongo_data_cache[name] or {}
			if mongo_data_cache[name] ~= nil then
				-- mongo_data_cache[name][id].lastsavetime = os.time()
				if mongo_data_cache[name][id]==nil then
					mongo_data_cache[name][id]={data=args}
				else
					mongo_data_cache[name][id].data = args
				end
				mongo_data_cache[name][id].lastsavetime = os.time()
				-- unilight.redis_sethashdata(name, tostring(id), table2json(args))
				-- unilight.redis_setdata(name..'_'..tostring(id),table2json(args))
				return
			end
			-- unilight.REDISDB.ExecuteCommand('SETEX',name..'_'..tostring(id),1800,table2json(args))
		end
		if name == "data" then
			mongo_data_cache[id]=nil
		end
	...
	-- 对指字的字段自增操作
	-----------------------------------------------------------------------
	unilight.incdate = function(name, id, args)
		local r = unilight.get_mongodb().IncCollectionFieldById(name, id,"", encode_repair(args))
		if r ~= nil and id ~= nil then
			local errorInfo = "unilight.incdate :" .. tostring(r)..'name:'..name
			if id ~= nil then
				errorInfo = errorInfo .. " _id:" .. id .. "   type is" .. type(id)
			end
			unilight.error(errorInfo)
	...
	unilight.savefield = function(name, id, fieldpath, data)
		if id == nil or type(id) == "userdata" or data == nil or type(data) == "userdata" then
			unilight.error("savefield id or data is null or type() is userdata")
	...
	--[[
		包装生成条件查询数组
		table.insert(filters,unilight.eq("_id",value._id))
	]]
	unilight.OrRange =function (filters)
		a = unilight.field("")
		return a.OrRange(filters)
	end

    --包装生成正则表达式
	unilight.RegEx =function (a, b)
		if type(a) == "string" then
			a = unilight.field(a)
		end
		return a.RegEx(b)
	...
	--[[
		...
		实例：查询多个玩家数据
        local orList = {}
        for _, uid in pairs({1001233, 1001232, 1001234}) do
        table.insert(orList, unilight.eq("uid", uid))
        end
...
--清理本地缓存
unilight.clearMongoCatch=function (uid)
	for key,_ in pairs(mongo_data_cache) do
		if Const.REDIS_CACHE_NAME[key]~=nil then
			-- print('disconnect')
			if mongo_data_cache[key][uid]==nil then return end
			-- print('delete',key,uid)
			local data = mongo_data_cache[key][uid].data
			unilight.get_mongodb().SaveCollectionById(key, encode_repair(data))
			--更新redis
			if unilight.REDISDB ~= nil then
				local keyName = key..'_'..tostring(data._id)
				-- print('keyName',keyName)
				unilight.REDISDB.ExecuteCommand('SETEX',keyName,1800,table2json(data))
			end
			mongo_data_cache[key][uid] = nil
		end
	end
end
--开启定时器,由redis初始完成后开启
unilight.Mongo_cache_ttl=function ()
	local timenow = os.time()
	for key,value in pairs(mongo_data_cache) do
		if Const.REDIS_CACHE_NAME[key]~=nil then
			for _id,val in pairs(value) do
				--设置超时时间
				if timenow -  val.lastsavetime >=800 then
					unilight.redis_setexpire(key..'_'..tostring(_id),1800)
					val.lastsavetime = timenow
				end
			end
		end
	end
