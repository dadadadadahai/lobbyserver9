-- 徽章等级相关gm请求
-- 请求徽章等级满足要求的个数
GmSvr.PmdRequestRewardActivitiesPmd_C = function(cmd, laccount)
    ...
    -- 根据领取内容判断
    if cmd.data.receivetype ~= nil and cmd.data.receivetype > 0 then
        filter = unilight.a(filter,unilight.eq("type",cmd.data.receivetype))
    end
    -- 根据发放时间判断
    ...
    local logInfos = unilight.chainResponseSequence(unilight.startChain().Table(nTask.DB_Log_Name).Filter(filter).OrderBy(order).Skip((cmd.data.curpage-1)*cmd.data.perpage).Limit(cmd.data.perpage))
    ...
    local infoNum = unilight.startChain().Table(nTask.DB_Log_Name).Filter(filter).Count()
    ...
    for _, logInfo in ipairs(logInfos) do
        table.insert(res.data.data,{
            ...
            receivetype     = logInfo.type,                                                 -- 净损失金额
            receivegold     = logInfo.gold,                                                 -- 领取返利金额
        })
        -- 统计合计数量
