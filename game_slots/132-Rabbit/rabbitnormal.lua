...
function PlayNormalGame(rabbitInfo,uid,betIndex,gameType)
    ...
    rabbitInfo.betgold = betgold

    -- 生成普通棋盘和结果
    -- local imageType = table_132_imagePro[gamecommon.CommRandInt(table_132_imagePro,'pro')].type
    -- imageType = GmProcess(imageType)
    local resultGame,realMul,imageType = gameImagePool.RealCommonRotate(uid,GameId,gameType,nil,Rabbit,{betchip=betgold,betIndex=betIndex,gameId=GameId,gameType=gameType,betchips=payScore})
    local winScore = 0
    -- 如果是普通则需要处理免费第一把信息
    if imageType == 2 then
        ...
        winScore = rabbitInfo.betgold * resultGame.winMul
...
function PlayNormalGameDemo(rabbitInfo,uid,betIndex,gameType)
    -- 清理棋盘附加信息
    rabbitInfo.iconsAttachData = {}
    -- 保存下注档次
    rabbitInfo.betIndex = betIndex
    ...
    local payScore = betgold * table_132_hanglie[1].linenum                                   -- 全部下注金额
    -- 扣除金额
    local _, ok = chessuserinfodb.WGoldChange(uid, Const.PACK_OP_TYPE.SUB, payScore, "兔子下注扣费")
    if ok == false then
        local res = {
            errno = 1,
            desc = "当前余额不足"
    ...
    local resultGame,realMul,imageType = gameImagePool.RealCommonRotate(uid,GameId,gameType,nil,Rabbit,{betchip=betgold,demo = IsDemo(uid),betIndex=betIndex,gameId=GameId,gameType=gameType,betchips=payScore})
    local winScore = 0
    -- 如果是普通则需要处理免费第一把信息
    if imageType == 2 then
        rabbitInfo.freeInfo = table.clone(resultGame.freeInfo)
