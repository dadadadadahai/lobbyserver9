-- 金牛游戏模块
module('NewGoldCow', package.seeall)
-- 金牛所需数据库表名称
DB_Name = "game149goldcow"
-- 金牛通用配置
GameId = 149
S = 70
W = 90
...
Table_Base = import "table/game/149/table_149_hanglie"                        -- 基础行列
MaxNormalIconId = 6
...
function GetBoards(uid,gameId,gameType,isFree,goldcowInfo)
    ...
    -- 随机是否触发福牛模式
    if goldcowInfo.bonusFlag == true or (math.random(10000) <= table_149_bonusPro[1].pro) or GmProcess("Free") then
        goldcowInfo.bonusFlag = true
        -- 初始化保底数量
        goldcowInfo.miniNum = goldcowInfo.miniNum or 0
        goldcowInfo.miniNum = goldcowInfo.miniNum + 1
        local iconId = table_149_bonusIconPro[gamecommon.CommRandInt(table_149_bonusIconPro, 'pro')].iconId
        boards = gamecommon.CreateSpecialChessData(DataFormat,NewGoldCow['table_149_free_'..gameType])
        -- 替换第一列和第三列
        local colNum = {1,3}
        ...
        -- 计算中奖倍数
        local winlines = gamecommon.WiningLineFinalCalc(boards,table_149_payline,table_149_paytable,wilds,nowild)
        -- 如果本轮中奖判断是否够保底
        if table.empty(winlines) == false and goldcowInfo.miniNum < 10 then
            local minimumGuaranteeFlag , resboards = GetMinimumGuarantee(gameType, goldcowInfo,winlines, uid)
            if minimumGuaranteeFlag then
                boards = resboards
                goldcowInfo.miniNum = 0
            end
        end
        if goldcowInfo.miniNum >= 10 then
    ...
    if (not goldcowInfo.bonusFlag) and (math.random(10000) <= table_149_mulPro[1].pro or GmProcess("Bonus"))  then
        fullIconFlag = true
        local iconId = table_149_mulIcon[gamecommon.CommRandInt(table_149_mulIcon, 'pro')].iconId
    ...
    if (not goldcowInfo.bonusFlag) and (not fullIconFlag) and #winlines == 0 then
        -- 判断是否假中奖
        if math.random(10000) <= table_149_mulPro[1].spuriousPro or GmProcess("False") then
            local iconId = table_149_mulIcon[gamecommon.CommRandInt(table_149_mulIcon, 'pro')].iconId
            -- 前两列相同图标
    ...
    for k, v in ipairs(winlines) do
        local addScore = v.mul * goldcowInfo.betMoney / table_149_hanglie[1].linenum
...
function GetMinimumGuarantee(gameType, goldcowInfo,reswinlines, uid)
    local winScore = 0
    ...
    for i = 1, 10 do
        -- 重新随机
        local iconId = table_149_bonusIconPro[gamecommon.CommRandInt(table_149_bonusIconPro, 'pro')].iconId
        local boards = gamecommon.CreateSpecialChessData(DataFormat,NewGoldCow['table_149_free_'..gameType])
        -- 替换第一列和第三列
        local colNum = {1,3}
        for _, col in ipairs(colNum) do
            for row = 1, DataFormat[col] do
        ...
        if mul == deficiencyMul then
            if chessuserinfodb.GetAHeadTolScore(uid) + goldcowInfo.betMoney * mul < userInfo.point.chargeMax then
                -- 返回保底数据
                return true ,boards
    ...
    for id, value in ipairs(mulRandomList) do
        if value.mul >= deficiencyMul then
            -- 返回保底数据
            if chessuserinfodb.GetAHeadTolScore(uid) + goldcowInfo.betMoney * value.mul < userInfo.point.chargeMax then
                -- 返回保底数据
    ...
    if chessuserinfodb.GetAHeadTolScore(uid) + goldcowInfo.betMoney * mulRandomList[1].mul < userInfo.point.chargeMax then
...
function GetResInfo(uid, goldcowInfo, gameType, tringerPoints)
    ...
    local res = {
        errno = 0,
        -- 是否满线
        bAllLine = table_149_hanglie[1].linenum,
        -- 获取玩家下注金额范围 下注配置
        betConfig = gamecommon.GetBetConfig(gameType,table_149_hanglie[1].linenum),
        -- 下注索引
        betIndex = goldcowInfo.betIndex,
        -- 全部下注金额
