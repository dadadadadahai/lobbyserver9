-- 老虎游戏模块
module('NewTiger', package.seeall)
-- 老虎所需数据库表名称
DB_Name = "game146tiger"
-- 老虎通用配置
GameId = 146
S = 70
W = 90
...
Table_Base = import "table/game/146/table_146_hanglie"                        -- 基础行列
MaxNormalIconId = 10
...
function GetBoards(uid,gameId,gameType,isFree,tigerInfo)
    ...
    res.bigWinIcon = -1
    -- 判断是否中respin
    if GmProcess().respin == true or math.random(10000) <= table_146_respinpro[1].pro then
        -- 中了respin
        ...
        if math.random(10000) <= table_146_mulPro[1].pro or GmProcess().bonus  then
            fullIconFlag = true
            local iconId = table_146_mulIcon[gamecommon.CommRandInt(table_146_mulIcon, 'pro')].iconId
        ...
        -- 计算中奖倍数
        local winlines = gamecommon.WiningLineFinalCalc(boards,table_146_payline,table_146_paytable,wilds,nowild)
        -- 计算中奖线金额
        for k, v in ipairs(winlines) do
            local addScore = v.mul * tigerInfo.betMoney / table_146_hanglie[1].linenum
...
function IsAllWinPoints(winlines)
    ...
    for _, winline in ipairs(winlines) do
        -- 对应的中奖列配置
        local pointInfo = table_146_payline[winline[1]]
...
function GetResInfo(uid, tigerInfo, gameType, tringerPoints)
    ...
    local res = {
        errno = 0,
        -- 是否满线
        bAllLine = table_146_hanglie[1].linenum,
        -- 获取玩家下注金额范围 下注配置
        betConfig = gamecommon.GetBetConfig(gameType,table_146_hanglie[1].linenum),
        -- 下注索引
        betIndex = tigerInfo.betIndex,
        -- 全部下注金额
