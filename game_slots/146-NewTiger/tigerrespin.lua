...
-- 计算respin最终棋盘
function RespinFinalBoards(tigerInfo)
    -- 获取W元素
    local wilds = {}
    wilds[W] = 1
    local nowild = {}
    local boards = {}
    local respinIconId = table_146_respinicon[gamecommon.CommRandInt(table_146_respinicon, 'pro')].iconId
    -- 空白图标判断
    ...
    for col = 1,#DataFormat do
        ...
        for row = 1,DataFormat[col] do
            local iconId = table_146_respin[gamecommon.CommRandInt(table_146_respin, 'pro')].result
...
function AheadRespin(datainfo,respinFinalResult)
    ...
    while true do
        ...
        local nowild = {}
        -- 计算中奖倍数
        local courseWinLines = gamecommon.WiningLineFinalCalc(respin.boards,table_146_payline,table_146_paytable,wilds,nowild)
        -- 计算中奖线金额
        for k, v in ipairs(courseWinLines) do
            local addScore = v.mul * datainfo.betMoney / table_146_hanglie[1].linenum
            table.insert(reswinlines[1], {v.line, v.num, addScore,v.ele})
        end
