...
function PlayNormalGameDemo(tigerInfo,uid,betIndex,gameType)
    -- 清理棋盘附加信息
    tigerInfo.iconsAttachData = {}
    -- 保存下注档次
    tigerInfo.betIndex = betIndex
    ...
    local payScore = betgold * table_127_hanglie[1].linenum                                   -- 全部下注金额
    -- 扣除金额
    local _, ok = chessuserinfodb.WGoldChange(uid, Const.PACK_OP_TYPE.SUB, payScore, "老虎下注扣费")
    if ok == false then
        local res = {
            errno = 1,
            desc = "当前余额不足"
    ...
    local resultGame,realMul,imageType = gameImagePool.RealCommonRotate(uid,GameId,gameType,nil,Tiger,{betchip=betgold,demo = IsDemo(uid),betIndex=betIndex,gameId=GameId,gameType=gameType,betchips=payScore})
    resultGame.winScore = realMul *  payScore
    -- 保存棋盘数据
    tigerInfo.boards = resultGame.boards
    tigerInfo.respin = resultGame.respin
    ...
    if resultGame.winScore >0 then 
      BackpackMgr.GetRewardGood(uid, Const.GOODS_ID.POINT, resultGame.winScore, Const.GOODS_SOURCE_TYPE.TIGER)
    end 
    -- 返回数据
    local res = GetResInfo(uid, tigerInfo, gameType)
    res.boards = {resultGame.boards}
