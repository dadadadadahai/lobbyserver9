...
Table_Base = import "table/game/150/table_150_hanglie"                        -- 基础行列
MaxNormalIconId = 6
...
function GetBoards(uid,gameId,gameType,isFree,rabbitInfo)
    ...
    -- 判断是否进入免费
    if isFree == false and  (math.random(10000) <= table_150_respinPro[1].pro or GmProcess()) then
        -- 普通则触发免费初始化
        rabbitInfo.free.totalTimes = table_150_respinPro[1].num                 -- 总次数
        rabbitInfo.free.lackTimes = table_150_respinPro[1].num - 1              -- 剩余游玩次数
    ...
    if isFree then
        local uNum = table_150_respinIcon[gamecommon.CommRandInt(table_150_respinIcon, 'pro')].uNum
        -- 判断保底
    ...
    -- 计算中奖倍数
    local winlines = gamecommon.WiningLineFinalCalc(boards,table_150_payline,table_150_paytable,wilds,nowild)
    ...
    for k, v in ipairs(winlines) do
        local addScore = v.mul * rabbitInfo.betMoney / table_150_hanglie[1].linenum
        ...
        for row = 1, DataFormat[col] do
            if boards[col][row] == U then
                if isMininumGuarantee then
                    ...
                    table.insert(iconsAttachData.boardsInfo,{col = col, row = row, mul = table_150_uPro[gamecommon.CommRandInt(table_150_uPro, 'pro')].mul})
                end
...
function GetMinimumGuarantee(uNum, rabbitInfo,uid)
    ...
    if deficiencyMul > 0 then
        ...
        for _, value in ipairs(table_150_respinIcon) do
        ...
        for i = 1, 10 do
            ...
            -- 随机图标倍数
            for id = 1, uNum do
                local mul = table_150_uPro[gamecommon.CommRandInt(table_150_uPro, 'pro')].mul
                sumMul = sumMul + mul
...
function GetResInfo(uid, rabbitInfo, gameType, tringerPoints)
    ...
    local res = {
        errno = 0,
        -- 是否满线
        bAllLine = table_150_hanglie[1].linenum,
        -- 获取玩家下注金额范围 下注配置
        betConfig = gamecommon.GetBetConfig(gameType,table_150_hanglie[1].linenum),
        -- 下注索引
        betIndex = rabbitInfo.betIndex,
        -- 全部下注金额
