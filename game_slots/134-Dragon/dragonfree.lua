...
function PlayFreeGame(dragonInfo,uid,gameType)
    ...
    if dragonInfo.free.lackTimes <= 0 then
        if dragonInfo.free.tWinScore > 0 then
            -- 获取奖励
            BackpackMgr.GetRewardGood(uid, Const.GOODS_ID.GOLD, dragonInfo.free.tWinScore, Const.GOODS_SOURCE_TYPE.DRAGON)
        end
    end
    -- 增加后台历史记录
...
--生肖龙免费游戏
function PlayFreeGameDemo(dragonInfo,uid,gameType)
    -- 清理棋盘信息
    dragonInfo.boards = {}
    -- 增加免费游戏次数
    dragonInfo.free.lackTimes = dragonInfo.free.lackTimes - 1
    ...
    if dragonInfo.free.lackTimes <= 0 then
        if dragonInfo.free.tWinScore > 0 then
            -- 获取奖励
            BackpackMgr.GetRewardGood(uid, Const.GOODS_ID.POINT, dragonInfo.free.tWinScore, Const.GOODS_SOURCE_TYPE.DRAGON)
        end
    end
