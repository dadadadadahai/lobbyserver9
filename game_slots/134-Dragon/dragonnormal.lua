...
function PlayNormalGame(dragonInfo,uid,betIndex,gameType)
    ...
    local res = {
        ...
        sumMulList =  table.sum(data.mulList),
        sumMul = data.sumMul,
...
function PlayNormalGameDemo(dragonInfo,uid,betIndex,gameType)
   -- 保存下注档次
    dragonInfo.betIndex = betIndex
    local betConfig = gamecommon.GetBetConfig(gameType,table_134_hanglie[1].linenum)
    local betgold = betConfig[dragonInfo.betIndex]                                                      -- 单注下注金额
    ...
    -- 扣除金额
    local remainder, ok = chessuserinfodb.WGoldChange(uid, Const.PACK_OP_TYPE.SUB, payScore, "生肖龙下注扣费")
    if ok == false then
        local res = {
            errno = 1,
            desc = "当前余额不足"
    ...
    dragonInfo.betgold = betgold
    -- 生成普通棋盘和结果
    local resultGame,realMul,imageType = gameImagePool.RealCommonRotate(uid,GameId,gameType,nil,Dragon,{betchip=betgold,demo = IsDemo(uid),betIndex=betIndex,gameId=GameId,gameType=gameType,betchips=payScore})
    ...
    if (imageType == 1 or imageType == 3 )and winscore > 0 then
        -- 增加奖励
        BackpackMgr.GetRewardGood(uid, Const.GOODS_ID.POINT, winscore, Const.GOODS_SOURCE_TYPE.DRAGON)
    elseif imageType == 2 then
        resultGame.isFree = true
        dragonInfo.free={
            totalTimes=8,
