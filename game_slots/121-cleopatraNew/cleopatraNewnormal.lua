...
function NormalDemo(gameType, betindex, datainfo, uid)
    ...
    datainfo.betindex = betindex
    --启用图库模式
    local alldisInfo,realMul ,imageType= gameImagePool.RealCommonRotate(uid,GameId,gameType,nil,cleopatraNew,{betchip=betMoney/LineNum, demo = IsDemo(uid),betIndex=betindex,gameId=GameId,gameType=gameType,betchips=chip})
    if  imageType == 3 then 
