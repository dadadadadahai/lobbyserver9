...
table_205_sessions = import 'table/game/205/table_205_sessions'
table_205_gailv = import 'table/game/205/table_205_gailv'
table_205_other = import 'table/game/205/table_205_other'
table_205_stock = import 'table/game/205/table_205_stock'

table_205ad_adnum = import 'table/game/205/table_205ad_adnum'
table_205ad_gold = import 'table/game/205/table_205ad_gold'
table_205ad_quit = import 'table/game/205/table_205ad_quit'
table_205ad_bTime = import 'table/game/205/table_205ad_bTime'
table_205ad_bNum = import 'table/game/205/table_205ad_bNum'
table_205ad_bili = import 'table/game/205/table_205ad_bili'
table_205ad_area = import 'table/game/205/table_205ad_area'
...
function Init()
    ...
    for _, v in ipairs(roomCfg) do
        ...
        MapRoom[gameType] = {
            ...
            Stock = table_205_sessions[gameType].initStock,
            RoomId = gameType,
            HisTory = {}, --历史数据 100把
            TableStatus = { status = statusDefine.Free, sTime = timeNow, eTime = table_205_other[gameType].FreeTime +timeNow }, --服务器处于的状态,启动时间，结束时间
            TableBets = { 0, 0, 0 },
            RealManBet = { 0, 0, 0 },
        ...
        for i = 1, 100 do
            local nid = table_205_gailv[gamecommon.CommRandInt(table_205_gailv, 'gailv')].ID
            table.insert(MapRoom[gameType].HisTory, nid)
        end
        gamecommon.RegisterStockDec(GameId, gameType, MapRoom[gameType], table_205_other[gameType])
    end
    --启动定时器
    unilight.addtimermsec("LongHu.Pluse", 500)
end
...
function Scene(gameType, laccount, uid)
    ...
    return {
        ...
        bets = table_205_other[gameType].bets, --下载配置
        ...
        -- playInfo = PlayInfo,
        betLow = table_205_sessions[gameType].betLow,
        leftSit = Table.leftSit,
        rightSit = Table.rightSit,
    }
end
function LongHuGetRankCmd_C(gameType,uid)
    local Table = MapRoom[gameType]
...
function Betting(gameType, betinfo, uid)
    ...
    if playinfo.IsRobot == false then
        if allBet<=0 then
            ...
            if chip < table_205_sessions[gameType].betLow then
                return {
                    errno = ErrorDefine.NOT_ENOUGHTCHIPS,
                    param = table_205_sessions[gameType].betLow,
                }
            end
        end
    end
    if Table.TableStatus.status ~= statusDefine.Betting then
        return {
            errno = ErrorDefine.LongHu_NotInBetting
        }
    end
    local area, betIndex = betinfo[1], betinfo[2]
    local betchip = table_205_other[gameType].bets[betIndex]
    if betchip == nil then
        return {
            errno = ErrorDefine.ERROR_PARAM
        }
    ...
    if (realBetArea[1] > 0 and area == 3) or (realBetArea[3] > 0 and area == 1) then
        return {
            errno = ErrorDefine.LongHu_OnlyArea
        }
    end
    local TableBets = Table.TableBets

    if allBet + betchip > table_205_sessions[gameType].maxBet then
        return {
            errno = ErrorDefine.LongHu_MAXBET
        }
    end
    -- 进入下注记录
    if playinfo.IsRobot == false then
    ...
    if chessuserinfodb.GetChargeInfo(uid) <= 0 then
        ...
        -- 扣除
        local _, ok = chessuserinfodb.WChipsChange(uid, Const.PACK_OP_TYPE.SUB, betchip, "龙虎斗玩法投注")
        if ok == false then
            return {
    ...
    if isSide then
        ...
        CmdMsgBrd('Cmd.LongHuSideBetCmd_S', brd, gameType)
    else
...
--批量下注
function LongHuBatchBetCmd_C(gameType, betinfo, uid)
    local chip = chessuserinfodb.RUserChipsGet(uid)
    if chip < table_205_sessions[gameType].betLow then
        return {
            errno = ErrorDefine.NOT_ENOUGHTCHIPS,
    ...
    if allBet > table_205_sessions[gameType].maxBet then
        return {
            errno = ErrorDefine.LongHu_MAXBET
        }
    end
    --进入充值判定
        if chessuserinfodb.GetChargeInfo(uid) <= 0 then
            return {
                errno = ErrorDefine.NO_RECHARGE,
                -- param = table_205_sessions[gameType].betCondition,
            }
        end
    ...
    --     }
    -- end
    local _, ok = chessuserinfodb.WChipsChange(uid, Const.PACK_OP_TYPE.SUB, betAllChip, "龙虎斗法投注")
    if ok == false then
        return {
            errno = ErrorDefine.CHIPS_NOT_ENOUGH
        }
...
function PluseDo(roomId, Table)
    ...
    if timeNow >= TableStatus.eTime then
        ...
        TableStatus.eTime = timeNow + table_205_other[roomId][statusStr]
        TriggerNext = true
        --触发广播阶段发送
        local doInfo = 'Cmd.LongHuChangeStatusCmd_S'
        local sdata = {
            errno = ErrorDefine.SUCCESS,
    ...
    if TableStatus.status == statusDefine.Betting or (TriggerNext and TableStatus.status == statusDefine.Sellte) then
        ...
        CmdMsgBrd('Cmd.LongHuBetListRoomCmd_S', boarddata, roomId)
        Table.TmpBet = {}
    end
...
function UserLeavel(uid, gameType)
    ...
    CmdMsgBrd('Cmd.LongHuUserLeavelCmd_S', send, gameType)
end
